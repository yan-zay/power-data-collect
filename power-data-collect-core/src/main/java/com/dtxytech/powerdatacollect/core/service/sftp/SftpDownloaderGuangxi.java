...
public class SftpDownloaderGuangxi extends SftpDownloader {
    ...
    public List<String> getAllFilePath(IndicatorTypeEnum indicatorType, ChannelSftp sftp, String remoteDir) {
        ...
        try {
            ...
            log.info("SftpDataSyncService syncFileList remoteDir:{}, entries.size:{}", remoteDir, entries.size());
    ...
    private void getTwoLvDir(IndicatorTypeEnum indicatorType, ChannelSftp sftp, String dir, List<String> filePaths) throws SftpException {
        ...
        for (ChannelSftp.LsEntry entry : entries) {
            ...
            if (entry.getAttrs().isDir() && isDateFormat(fileName)) {
                String dateDir = dir + SEPARATOR + fileName;
                // 在日期目录中查找CDQYC和DQYC文件
                collectFilePath(sftp, dateDir, indicatorType, filePaths);
            }
        }
    }
     ...
     * 从日期目录收集文件路径
     */
    private void collectFilePath(ChannelSftp sftp, String dateDir, IndicatorTypeEnum indicatorType, List<String> filePaths) {
        log.info("SftpDownloaderGuangxi collectFilePath dateDir:{}", dateDir);
        try {
